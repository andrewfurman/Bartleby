...
- For The World in Brief greeting, prefer structured RSS time (`<updated>` or non-midnight `pubDate`) before the article body's `updated X ago` marker; midnight UTC `pubDate` values are treated as date placeholders, not spoken as 8 p.m. Eastern updates.
...
BARTLEBY_TWILIO_BOOTSTRAP_TIMEOUT_MS=12000
